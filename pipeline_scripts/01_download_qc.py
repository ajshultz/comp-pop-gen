...
def fastq_trim_align_stats(sp_dir,sra,sp_abbr,sample):
    ...
    cmd_1 = 'module load NGmerge/0.1-fasrc01\nmodule load bwa/0.7.9a-fasrc03\nmodule load java/1.8.0_45-fasrc01'
    
    #If paired end data:
    #Trim adapters with NGmerge
    cmd_2 = 'NGmerge -1 %s/fastq/%s_1.fastq.gz -2 %s/fastq/%s_2.fastq.gz -a -v -o %s/fastq/%s_trimmed -n 8 -z'%(sp_dir,sra,sp_dir,sra,sp_dir,sra)
    
    #Map to genome with BWA mem
    cmd_3 = r"bwa mem -M -t 8 -R '%s' %s/fastq/%s_trimmed_1.fastq.gz %s/fastq/%s_trimmed_2.fastq.gz > %s/alignment/%s_bwa.sam"%(read_group_info,sp_dir,sra,sp_dir,sra,sp_dir,sra)
    
    cmd_4 = 'java -Mxm7g -XX:ParallelGCThreads=6 -jar %s SortSam I=%s/alignment/%s.bam O=%s/alignment/%s.sorted.bam SORT_ORDER=coordinate'%(path_to_picard,sp_dir,sra,sp_dir,sra)
    
    cmd_5 = 'java -Mxm7g -XX:ParallelGCThreads=6 -jar %s BuildBamIndex I=%s/alignment/%s.sorted.bam'%(path_to_picard,sp_dir,sra)
    
    #Calculate alignment summary stats
    cmd_6 = 'java -Mxm7g -XX:ParallelGCThreads=6 -jar %s CollectAlignmentSummaryMetrics I=%s/alignment/%s.sorted.bam R=%s/genome/%s.fa METRIC_ACCUMULATION_LEVEL=SAMPLE O=%s/stats/%s.%s.alignment_metrics.txt'%(path_to_picard,sp_dir,sra,sp_dir,sp_abbr,sp_dir,sample,sra)

    cmd_7 = 'if [%s/stats/%s.%s.alignment_metrics]\nrm %s/alignment/%s_bwa.sam \nfi'%(sp_dir,sample,sra,sp_dir,sra)
    ...
    #Format sbatch script and write file
    slurm_script = script_create()
    genome_script = slurm_script.format(partition="shared",time="2-0:00",mem="16000",cores="8",nodes="1",jobid="Trim_Map",cmd=final_cmd)
