...
def genotypegvcf_sbatch(sp_dir,sp_abbr,sample_list,het,nintervals,memory_gg,combine_gvcf_program):
    ...
    if combine_gvcf_program == "CombineGVCFs":
        cmd_3 = '/n/home13/ashultz/sw/progs/gatk-4.0.8.1/gatk --java-options "-Xmx${MEM}g -XX:ParallelGCThreads=1" CombineGVCFs -R %s/genome/%s.fa %s -O %s/gvcf/%s.${SLURM_ARRAY_TASK_ID}.g.vcf.gz --intervals %s/genome/%s_splits_interval_lists/%s_${SLURM_ARRAY_TASK_ID}.interval_list'%(sp_dir,sp_abbr,all_sample_variant_call,sp_dir,sp_abbr,sp_dir,nintervals,sp_abbr)
