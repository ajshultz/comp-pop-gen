...
def sbatch_submit(filename,memory,timelimit):
    proc = Popen('sbatch %s --mem=%s --time=%s'%(filename,memory,timelimit),shell=True,stdout=PIPE,stderr=PIPE)
    ...
    for sample in sample_dict.keys():
        ...
        if os.path.isfile(dedup_filename) or os.path.isfile(dedup_sorted_filename):
            ...
            if coverage_dict[sample] < 0.95:
                cmd_2 = 'gatk --java-options "-Xmx%dg -XX:ParallelGCThreads=1" DownsampleSam -I %s/%s.dedup.sorted.bam -O %s/dedup/%s.%sX.dedup.sorted.bam -S Chained -P %f -A 0.0001 --COMPRESSION_LEVEL 5'%((int(memory_ds)-2),orig_dir,sample,sp_dir,sample,coverage,coverage_dict[sample])
            else:
